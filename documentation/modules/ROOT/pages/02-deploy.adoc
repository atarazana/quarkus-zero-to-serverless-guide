...
----
cd $CHE_PROJECTS_ROOT/quarkus-zero-to-serverless-guide/atomic-fruit-service
----
...
[source,sh,role="copypaste"]
----
unset MAVEN_CONFIG && ./mvnw -DskipTests clean package
----
...
[source,sh,role="copypaste"]
----
chmod +x ./target/atomic-fruit-service-1.0-SNAPSHOT-runner && \
...
. Navigate to `src → main → java → {group_id}` in the project tree starting at folder `{artifact_id_quarkus}` and double click on `FruitResource.java`. Update with the relevant lines bellow replacing all the existing code as of `import javax.ws.rs.core.MediaType;` till the end of the class:
+
...
. Again and if you already closed it, navigate to `src → main → java → {group_id}` and double click on `FruitResource.java`. Update with the following code  replacing all the existing code as of `import org.jboss.logging.Logger; // logging` till the end of the class:
+
[source,java,role="copypaste"]
----
