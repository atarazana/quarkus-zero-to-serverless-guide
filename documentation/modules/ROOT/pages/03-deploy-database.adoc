...
Let’s modify the application and add a companion bean. In Dev Spaces, navigate to `src → main → java → {group_id}`, right-click on `fruit` folder in the project browser and select _New_ -> _File_. 
...
Use the public link provided on the area called **, click on arrow icon as indicated in the next picture.
